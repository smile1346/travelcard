...
public class ExchangeRateService {
    ...
    public UUID saveExchangeRateAndQuotations(ExchangeRateRequest req) throws JsonProcessingException {
        ...
        cacheExchangeRateInRedis(yyyymm, req);
    ...
    public ExchangeRateWithQuotationsResponse getExchangeRateById(UUID exchangeRateId) {
        ...
        try {
            List<ExchangeRateDetail> exchangeRates = objectMapper.readValue(
                row[5].toString(),
                new TypeReference<List<ExchangeRateDetail>>() {}
            );
    ...
    public List<ExchangeRateDetail> getCachedExchangeRates(String yyyymm, LocalDate date, int round) {
        String key = String.format("exchange_rate:%s:%s:%d", yyyymm, date, round);
        String json = redisTemplate.opsForValue().get(key);
        if (json == null) return null;

        try {
            return objectMapper.readValue(json, new TypeReference<List<ExchangeRateDetail>>() {});
        } catch (JsonProcessingException e) {
            throw new RuntimeException("Failed to deserialize exchange rates from Redis", e);
        }
    }

    private void cacheExchangeRateInRedis(String yyyymm, ExchangeRateRequest req) {
        String key = String.format("exchange_rate:%s:%s:%d", yyyymm, req.getAnnouncementDate(), req.getRound());
        try {
            String value = objectMapper.writeValueAsString(req.getExchangeRates());
            redisTemplate.opsForValue().set(key, value);
        } catch (JsonProcessingException e) {
            throw new RuntimeException("Failed to serialize exchange rate data for Redis", e);
        }
    }
